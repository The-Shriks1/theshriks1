...
export function TypeScape() {
  ...
  const xLeft = useTransform(scrollYProgress, [0, 1], prefersReducedMotion ? ["-2vw", "-2vw"] : ["-2vw", "-22vw"]);
  const xRight = useTransform(scrollYProgress, [0, 1], prefersReducedMotion ? ["2vw", "2vw"] : ["2vw", "22vw"]);
  const opacity = useTransform(scrollYProgress, [0.0, 0.2, 0.8, 1], [0, 1, 1, 0]);
  const blur = useTransform(scrollYProgress, [0, 0.3, 0.7, 1], prefersReducedMotion ? [0, 0, 0, 0] : [4, 0, 0, 6]);
  const rule = useTransform(scrollYProgress, [0, 1], prefersReducedMotion ? ["100%", "100%"] : ["0%", "100%"]);
